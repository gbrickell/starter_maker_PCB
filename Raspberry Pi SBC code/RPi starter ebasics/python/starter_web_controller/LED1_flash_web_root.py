#!/usr/bin/python
# Starter PCB PCB version 1.0
# file name: LED1_flash_web_root.py
...
#  - this version of the app is for use by a user with root privileges (i.e. sudo) and uses HTTP port 80
# 
...
# command to run: sudo python3 ./starter_maker_PCB1/RPi_code/starter_ebasics/starter_web_controller/LED1_flash_web_root.py
#
...
#   as used below - without sudo a non-standard port must be used e.g. 8000
##################################################################################

if __name__ == "__main__":
    run_electronics_app01.run(host='0.0.0.0', port=80, debug=False, threaded=True) # 0.0.0.0 means any device on the network can access the web app
    # uses port 80 so you must run the app as root (ie use sudo) which is necessary if the 'production' port 80 is used

    # debug version
    #run_electronics_app01.run(host='0.0.0.0', port=80, debug=True)   # 0.0.0.0 means any device on the network can access the web app
