# Starter Maker Kit version of LED_flash.py that flashes a the red, amber & green LEDs on/off sequentially
# Run from Thonny using the file that may be stored at /home/pi/starter_maker_PCB1/Pico_MPcode/LED_red_amber_green_flash.py
...
amber_positive_pin = 11   # this is the GPIO pin that the AMBER LED positive leg (via the resistor) is connected to
...
amberled = machine.Pin(amber_positive_pin, machine.Pin.OUT)
...
try: # this loop is not strictly necessary but it does allow the script to 'clean up' after a CTRL-C
    while True:  # this is the loop that continuously flashes the LED on and off every half second
        ...
        amberled.value(0)  # amber LED switched off by making the GPIO go LOW
        ...
        amberled.value(1)  # amber LED switched on by making the GPIO go LOW
        greenled.value(0)  # green LED switched off by making the GPIO pin go HIGH
    ...
    amberled.value(0)                               # set the GPIO pin LOW
    machine.Pin(amber_positive_pin, machine.Pin.IN) # now make the pin an INPUT
