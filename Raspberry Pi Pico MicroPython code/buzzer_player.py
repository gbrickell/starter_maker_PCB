...
def buzz(frequency, length):	 #create the function "buzz" and feed it the pitch and duration)
    ...
    for i in range(numCycles):		   #start a loop from 0 to the variable "cycles" calculated above
        ...
        utime.sleep(delayValue)		   #wait with buzzer pin high
        buzzer.value(0)                #set buzzer pin to low
        utime.sleep(delayValue)		   #wait with buzzer pin low
	

def destroy():
    buzzer.value(0)                          # set the GPIO pin LOW
    machine.Pin(buzzer_pin, machine.Pin.IN)  # now make the pin an INPUT
